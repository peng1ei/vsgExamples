    ...
    void apply(vsg::KeyPressEvent& keyPress) override
    {
        if (keyPress.keyBase == 'i' && lastPointerEvent)
        {
            ...
            if (lastIntersection)
            {
                ...
                info.position = vsg::vec3(
                    static_cast<float>(lastIntersection.worldIntersection.r),
                    static_cast<float>(lastIntersection.worldIntersection.g),
                    static_cast<float>(lastIntersection.worldIntersection.b)
                ) - info.dimensions * 0.5f;
