...
namespace mvsg {
    ...
    public:
        ...
        template<class R>
        observer_ptr(R *ptr) 
            : _auxiliary(ptr ? ptr->getOrCreateAuxilary() : nullptr ) {
        }
        ...
        bool valid() const {
            return _auxiliary.valid() && _auxiliary->getConnectedObject() != nullptr;
        }

        bool operator !() const {
            return !_auxiliary || _auxiliary->getConnectedObject() == nullptr;
        }
